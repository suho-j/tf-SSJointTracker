...
def SQL_CREATETABLE_Angle(tablename=TABLENAME):
    # distance values call (humans list length = people).
    # NOSE = 0
    # NECK = 1
    # RSHOULDER = 2
    ...
    query_string = ('create table ' + tablename+ '('
                                                  ' ID varchar(30),'
                                                  ' RSHOULDER_ float(10),   RELBOW float(10),'
                                                  ' LSHOULDER float(10),    LELBOW float(10),'
                                                  ' RHip float(10),         RKNEE float(10), '
                                                  ' LHip float(10),         LKNEE float(10), '
                                                  ' PRIMARY KEY (id)'
                                                  ')')
    SQL_EXECUTE(query_string, tablename)

def SQL_insertArr_Angle(arr_Angle):
    arr_return = []
    for index in range(8):
        arr_return.append(str(arr_Angle[index]))

    return arr_return

def SQL_INSERT_Angle(arr_Angle, tablename=TABLENAME):
    varlist = SQL_insertArr_Angle(arr_Angle)
    var_string = ', '.join(varlist)
    print("var_str : "+ var_string)
