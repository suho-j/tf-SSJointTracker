...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, joint_pixelX, joint_pixelY, human_match, depth_data_array, imgcopy=False):
        ...
        for human in humans:
            ...
            neck[k] = int(depth_data_array[human.body_parts[1].y * image_w + 0.5][human.body_parts[1].x * image_w + 0.5])
            ...
            # if True:
                for i in range(common.CocoPart.Background.value):
                    ...
                    if not joint_pixelX[j][i] == 1:
                        joint_pixelX[j][i] = int(
                            joint_pixelX[j][i] * weight + int(body_part.x * image_w + 0.5) * (1 - weight))
                        joint_pixelY[j][i] = int(
                            joint_pixelY[j][i] * weight + int(body_part.y * image_h + 0.5) * (1 - weight))
                    else:
            ...
            #             joint_pixelY[human_section][i] = int(body_part.y * image_h + 0.5)
                ...
                for pair_order, pair in enumerate(common.CocoPairsRender):
                    ...
                    if wrist_check == 1 and (pair[0] == 1 or pair[0] == 8 or pair[0] == 11):
                        ...
                        npimg = cv2.line(npimg, centers[pair[0]], wrist_inference, (rgb[human_section][0], rgb[human_section][1], rgb[human_section][2]), 3)
                    ...
                    npimg = cv2.line(npimg, centers[pair[0]], centers[pair[1]], (rgb[human_section][0], rgb[human_section][1], rgb[human_section][2]), 3)
